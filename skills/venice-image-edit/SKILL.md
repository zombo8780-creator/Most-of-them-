...
- All endpoints return the edited image as binary by default; some accept `return_binary: false` to get base64 JSON.
...
| Field | Notes |
|---|---|
| `model` | Default `qwen-edit`. `modelId` is deprecated alias. |
...
| Code | Cause |
|---|---|
| `400` | Bad params — image dims out of range, file too large, unknown model, unsupported aspect ratio for the model. |
| `401` | Auth or Pro-only model. |
| `402` | Insufficient balance. `INSUFFICIENT_BALANCE` (Bearer) / `PAYMENT_REQUIRED` (x402). |
| `413` | Payload too large (> 25 MB). |
| `422` | Content policy violation. May include `suggested_prompt`. |
| `429` | Rate limited. |
